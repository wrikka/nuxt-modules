...
export default defineNuxtModule<WpaymentModuleOptions>({
  ...
  setup(options, nuxt) {
    ...
    // Register plugins
    addPlugin(resolver.resolve('./runtime/plugins/wpayment.client'));
    addPlugin(resolver.resolve('./runtime/plugins/wpayment.server'));
    ...
    if (options.workos) {
      ...
      nuxt.options.alias['#workos'] = workosResolver.resolve('./runtime/workos');
      nuxt.options.alias['#workos/types'] = workosResolver.resolve('./runtime/workos/shared/types/index');
      nuxt.options.alias['#workos/composables'] = workosResolver.resolve('./runtime/workos/composables/index');
      nuxt.options.alias['#workos/utils'] = workosResolver.resolve('./runtime/workos/utils/index');

      // Register WorkOS composables
      addImportsDir(workosResolver.resolve('./runtime/workos/composables'));

      // Register WorkOS plugins
      addPlugin(workosResolver.resolve('./runtime/workos/plugins/workos.client'));
      addPlugin(workosResolver.resolve('./runtime/workos/plugins/workos.server'));

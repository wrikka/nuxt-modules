...
export default defineNuxtModule<FeatureFlagsModuleOptions>({
  ...
  setup(options, nuxt) {
    ...
    // Register plugins
    addPlugin(resolver.resolve('./runtime/plugins/feature-flags'));

    // Add server API handlers
    addServerHandler({
      route: '/api/feature-flags',
      handler: resolver.resolve('./runtime/server/api/flags.get'),
    });

    addServerHandler({
      route: '/api/feature-flags/evaluate',
      handler: resolver.resolve('./runtime/server/api/evaluate.post'),
    });
    ...
    addServerHandler({
      ...
      handler: resolver.resolve('./runtime/server/api/experiments/index.get'),
    });

    addServerHandler({
      route: '/api/feature-flags/experiments/metrics',
      handler: resolver.resolve('./runtime/server/api/experiments/metrics.post'),
    });
    ...
    addServerHandler({
      ...
      handler: resolver.resolve('./runtime/server/api/simulate.post'),
    });

    addServerHandler({
      route: '/api/feature-flags/impact',
      handler: resolver.resolve('./runtime/server/api/impact.post'),
    });

    addServerHandler({
      route: '/api/feature-flags/bulk',
      handler: resolver.resolve('./runtime/server/api/bulk.post'),
    });

    addServerHandler({
      route: '/api/feature-flags/dependencies',
      handler: resolver.resolve('./runtime/server/api/dependencies.get'),
    });
    ...
    addServerHandler({
      ...
      handler: resolver.resolve('./runtime/server/api/env-diff.post'),
    });

    addServerHandler({
      route: '/api/feature-flags/annotations',
      handler: resolver.resolve('./runtime/server/api/annotations.get'),
    });

  ...
  <aside class="w-80 bg-gray-100 p-4 overflow-y-auto">
    ...
    <div v-if="selectedBlock">
      <div v-for="(prop, key) in selectedBlock.props" :key="key" class="mb-4">
        ...
        <input v-if="typeof prop === 'string'" type="text" :id="`prop-${key}`" :value="prop" @input="$emit('update-prop', { key, value: $event.target.value })" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
...
<script setup>
defineProps({
  selectedBlock: { type: null, required: true }
});

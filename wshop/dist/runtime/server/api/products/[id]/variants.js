...
export default defineEventHandler(async (event) => {
  ...
  if (method === "PUT") {
    ...
    const [updatedVariant] = await db.update(productVariants).set(validation.data).where(and(eq(productVariants.id, variantId), eq(productVariants.productId, productId))).returning();

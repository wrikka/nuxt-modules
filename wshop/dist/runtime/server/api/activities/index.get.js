...
export default defineEventHandler(async () => {
  try {
    ...
    const orderActivities = recentOrders.map((order) => ({
      ...
      user: order.customer ? { name: order.customer.name, email: order.customer.email } : void 0
    }));
    ...
    const sortedActivities = combinedActivities.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime()).slice(0, 10);

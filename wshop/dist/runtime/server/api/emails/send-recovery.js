...
export default defineEventHandler(async (event) => {
  ...
  const template = { html: `<body>TODO: Email template. Checkout URL: ${checkoutUrl}</body>` };
  try {
    ...
    await db.update(carts).set({ recoveryEmailSentAt: /* @__PURE__ */ new Date() }).where(eq(carts.id, cartId));
    return { success: true };
  } catch (error) {

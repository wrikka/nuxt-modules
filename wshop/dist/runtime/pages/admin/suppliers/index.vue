...
const availableSuppliers = [
  { id: "printful", name: "Printful", icon: "\u{1F3A8}", description: "Print-on-demand service" },
  { id: "printify", name: "Printify", icon: "\u{1F5A8}\uFE0F", description: "Print-on-demand service" },
  { id: "spocket", name: "Spocket", icon: "\u{1F4E6}", description: "Dropshipping service" },
  { id: "modalyst", name: "Modalyst", icon: "\u{1F6CD}\uFE0F", description: "Dropshipping service" }
];
const isConnected = (supplierId) => {
  return suppliers.value.some((s) => s.id === supplierId && s.enabled);
};
const connectSupplier = async (supplierId) => {
  try {
...
const configureSupplier = (supplierId) => {

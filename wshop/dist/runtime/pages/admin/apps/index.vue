      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <div class="h-40 bg-gray-200 flex items-center justify-center">
            <div class="text-4xl">{{ app.icon || "\u{1F4E6}" }}</div>
          </div>
          <div class="p-4">
...
<script setup>
const { data: apps, pending, error, refresh } = await useFetch("/api/apps");
const { data: installedApps, refresh: refreshInstalled } = await useFetch("/api/apps/installed");
...
const installApp = async (appId) => {
  try {
    ...
    console.error("Failed to install app:", error2);
  }
};
const uninstallApp = async (installedAppId) => {
  try {
    await $fetch(`/api/apps/${installedAppId}`, { method: "DELETE" });
    await refreshInstalled();
  } catch (error2) {
    console.error("Failed to uninstall app:", error2);
  }
};
</script>

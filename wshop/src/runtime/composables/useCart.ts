...
import { useDiscountManager } from "./cart/useDiscountManager"
import { useGiftCardManager } from "./cart/useGiftCardManager"
import { useTaxCalculator } from "./cart/useTaxCalculator"

export const useCart = () => {
	const { data: cart, pending, error, refresh } = useAsyncData<Cart>(
		"cart",
		() => $fetch<Cart>("/api/cart"),
		{ deep: false },
	)
	...
	const itemCount = computed(() =>
		cart.value?.items?.reduce((total: number, item: CartItem) => total + item.quantity, 0) || 0
	)
	...
	const shipping = computed(() => 0) // Placeholder

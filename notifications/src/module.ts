...
export default defineNuxtModule<NotificationsModuleOptions>({
  ...
  setup(options, nuxt) {
    ...
    // Register plugins
    addPlugin(resolver.resolve('./runtime/plugins/notifications'));
    ...
    addServerHandler({
      route: '/api/notifications',
      handler: resolver.resolve('./runtime/server/api/index.post'),
      method: 'post',
    });

    addServerHandler({
      route: '/api/notifications/:id/read',
      handler: resolver.resolve('./runtime/server/api/read.put'),
      method: 'put',
    });

    addServerHandler({
      route: '/api/notifications/preferences',
      handler: resolver.resolve('./runtime/server/api/preferences.get'),
    });

...
export default defineNuxtModule({
  ...
  setup(options, nuxt) {
    ...
    // Register plugins
    addPlugin(resolver.resolve('./app/plugins/onboarding'));

    // Add server API handlers
    addServerHandler({
      route: '/api/onboarding/status',
      handler: resolver.resolve('./app/server/api/status.get'),
    });

    addServerHandler({
      route: '/api/onboarding/complete',
      handler: resolver.resolve('./app/server/api/complete.post'),
      method: 'post',
    });

    addServerHandler({
      route: '/api/onboarding/skip',
      handler: resolver.resolve('./app/server/api/skip.post'),
      method: 'post',
    });

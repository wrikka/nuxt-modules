...
import { useVideoRecordingExport } from "~/composables/useVideoRecordingExport";
...
const exportManager = useVideoRecordingExport();

onMounted(() => {
	exportManager.loadAllRecordings();
});
...
const handleExport = async (rec: import("./types").Recording, options: import("~/composables/useVideoExport").RecordingExportOptions) => {
	await exportManager.handleExport(rec, options);
};

const handleTrimSave = async (blob: Blob, name: string) => {
	await exportManager.handleTrimSave(blob, name);
};
	...
	<div class="space-y-6">
		...
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
			<div class="lg:col-span-2 space-y-6">
				...
				<div v-if="activeTab === 'settings'" class="space-y-6">
					...
					<RecordingControls
						...
						:recordings="exportManager.recordings.value"
						@start="recording.startRecording"
						@stop="recording.stopRecording"
						@pause="recording.pauseRecording"
						@resume="recording.resumeRecording"
						@download="exportManager.downloadRecording"
						@upload="exportManager.uploadRecording"
						@delete="exportManager.deleteRecording"
					/>
				</div>
			...
			<div class="lg:col-span-1 space-y-6">
				<RecordingStats :recordings="exportManager.recordings.value" />
				<RecordingsList
					:recordings="exportManager.recordings.value"
					@preview="exportManager.previewRecording"
					@download="exportManager.downloadRecording"
					@upload="exportManager.uploadRecording"
					@delete="exportManager.deleteRecording"
				/>
				<BatchExport :recordings="exportManager.recordings.value" @export="console.log" />
			</div>
		</div>

		<!-- Export Dialog -->
		<ExportDialog
			v-if="exportManager.selectedRecording.value"
			v-model:isOpen="exportManager.showExportDialog.value"
			:recording="exportManager.selectedRecording.value"
			@export="handleExport(exportManager.selectedRecording.value!, $event)"
		/>

		<!-- Trim Editor -->
		<TrimEditor
			v-if="exportManager.selectedRecording.value"
			v-model:isOpen="exportManager.showTrimEditor.value"
			:recording="exportManager.selectedRecording.value"
			@save="handleTrimSave"
		/>
	</div>

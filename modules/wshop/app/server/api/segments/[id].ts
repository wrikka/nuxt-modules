...
export default defineEventHandler(async (event) => {
	...
	const segmentId = event.context.params?.id as string
	...
	if (method === "PUT") {
		...
		const updatedSegment = await db.transaction(async (tx) => {
			// Update segment name/description if provided
			if (name || description) {
			...
			// If rules are provided, replace all existing rules with the new ones
			if (rules) {
				// 1. Delete old rules
			...
			// Fetch and return the updated segment with its relations

// d:/wshop/app/server/api/discounts/[id].ts
...
export default defineEventHandler(async (event) => {
	...
	const discountId = event.context.params?.id as string

	if (!discountId) {
		throw createError({ statusCode: 400, statusMessage: "Discount ID is required" })
	}

	// GET: Fetch a single discount
	if (method === "GET") {
		try {
	...
	if (method === "DELETE") {
		try {
			const deletedDiscount = await db.delete(discounts).where(eq(discounts.id, discountId))
				.returning()

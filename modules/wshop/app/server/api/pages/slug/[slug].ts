// d:/wshop/server/api/pages/slug/[slug].ts
...
export default defineEventHandler(async (event) => {
	...
	const locale = query.locale as string || "en" // Default to 'en' if not provided
	...
	try {
		const page = await db.query.pages.findFirst({
			where: and(
				...
				eq(pages.status, "published"), // Only return published pages
			),
		})

...
import type { LoyaltyPoint } from '~~/shared/types'
...
const getPointIcon = (type: LoyaltyPoint['type']) => {
  switch (type) {
    case 'earned':
...
const getPointColor = (type: LoyaltyPoint['type']) => {
  switch (type) {
    case 'earned':
...
const getPointLabel = (type: LoyaltyPoint['type']) => {
  switch (type) {
    case 'earned':
      return 'ได้รับ'
    case 'redeemed':
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="text-right">
            <div 
              :class="[
                'text-lg font-bold',
                point.type === 'earned' ? 'text-green-600' : 
                point.type === 'redeemed' ? 'text-red-600' : 'text-gray-600'
              ]"
            >

...
import type { CategoryError } from "./types"

export function useCategoryApi() {
	const handleApiError = (err: unknown, defaultMessage: string) => {
		const error = err as CategoryError
	...
	const updateCategory = async (id: number, updates: Partial<Category>) => {
		try {
			return await $fetch<Category>(`/api/categories/${id}`, {
				method: "put",
				body: updates,
			})
		} catch (err) {
	...
	const exportCategories = async (format: "csv" | "json" = "csv") => {
		try {
	...
	const importCategories = async (file: File) => {
		try {
			...
			return await $fetch<Category[]>("/api/categories/import", {
				method: "post",
				body: formData,
			})
		} catch (err) {

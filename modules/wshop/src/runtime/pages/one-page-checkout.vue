...
onMounted(async () => {
  try {
    stripe.value = await loadStripe(config.public.stripePublishableKey as string);
    if (!stripe.value) throw new Error('Stripe failed to load.');

    const intent = await $fetch<{ clientSecret: string }>('/api/stripe/payment-intent', {
      method: 'POST',
    ...
    errorMessage.value = (error as { data?: { message?: string }; message?: string }).data?.message || (error as Error).message || 'An unexpected error occurred.';
  }
});

// Watch for address changes to recalculate taxes
watch(shippingInfo.value.address, (newAddress) => {
  if (newAddress && newAddress.country && newAddress.zipCode) {
    const addressForTaxes: Omit<Address, 'id' | 'type' | 'customerId' | 'firstName' | 'lastName' | 'phone'> = {
    ...
    calculateTaxes(addressForTaxes as unknown as Address);
  }
}, { deep: true });
...
const handleSubmit = async () => {
  ...
  // First, update payment intent with customer/shipping details
...
<template>
  <div class="container mx-auto p-4 md:p-8">
    <div v-if="!cart || (cart as any).itemCount === 0" class="text-center">
      ...
      <div class="bg-gray-50 p-6 rounded-lg shadow-sm h-fit sticky top-8">
        ...
        <div class="space-y-2">
          <div v-for="item in cart.items" :key="item.id" class="flex justify-between text-sm">
            ...
            <span>${{ (parseFloat(item.price) * (item as any).quantity).toFixed(2) }}</span>
          </div>
        </div>

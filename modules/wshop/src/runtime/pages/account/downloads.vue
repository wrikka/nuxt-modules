...
const { data: products, pending, error } = await useFetch<DownloadableProduct[]>('/api/account/downloads');

const handleDownload = async (productId: string, fileId: string) => {
  try {
    // This API call verifies the user's right to download and returns the URL
    const response = await $fetch<{ downloadUrl: string }>(`/api/products/${productId}/download/${fileId}`);

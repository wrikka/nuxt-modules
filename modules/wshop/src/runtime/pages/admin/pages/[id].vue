  ...
  <div class="h-screen flex flex-col">
    ...
    <div class="flex-grow flex overflow-hidden">
      ...
      <div class="w-[350px] flex-shrink-0 overflow-y-auto bg-gray-50">
          <PropsInspector :selected-block="selectedBlock || undefined" @update-prop="updateProp" />
          <SeoEditor 
            :title="page.title" 
            :slug="page.slug" 
...
const page = ref<Page | Omit<Page, 'id' | 'createdAt' | 'updatedAt' | 'status' | 'locale'>>({
  title: 'New Page',
  slug: '',
  content: [],
  metaTitle: '',
...
const metaTitle = computed({
  ...
  set: (value) => { page.value.metaTitle = value; }
});

const metaDescription = computed({
  get: () => page.value.metaDescription || '',
  set: (value) => { page.value.metaDescription = value; }
});
...
const updateProp = ({ key, value }: { key: string; value: string | number | boolean | null | undefined }) => {
...
onMounted(async () => {
  if (!isNew) {
    try {
      const data = await $fetch<Page>(`/api/pages/${pageId}`);
      if (data) {
        page.value = data;
      }
    } catch (error) {

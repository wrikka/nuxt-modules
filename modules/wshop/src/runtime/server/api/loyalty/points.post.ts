...
const addPointsSchema = z.object({
	...
	programId: z.string(), // Assuming a single loyalty program for now
})
...
export default defineEventHandler(async (event) => {
	...
	try {
		const [updatedLoyalty] = await db.update(customerLoyalty)
			.set({
		...
		if (!updatedLoyalty) {
			// Optionally, create a new customer loyalty record if one doesn't exist

...
export function useSlideDeck(deckData?: SlideDeck) {
	...
	return {
		...
		updateSpotlightPosition: (x: number, y: number) =>
			(spotlightPosition.value = { x, y }),
		toggleLaser: () => (isLaserActive.value = !isLaserActive.value),
		updateLaserPosition: (x: number, y: number) =>

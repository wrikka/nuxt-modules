...
export default defineNuxtModule<WformModuleOptions>({
  ...
  setup(options, nuxt) {
    ...
    // Register plugin
    addPlugin(resolver.resolve('./app/plugins/wform'));
    ...
    addComponent({
      ...
      filePath: resolver.resolve('./app/components/WForm.vue'),
      global: options.components?.global,
    });

    addComponent({
      name: `${componentPrefix}Field`,
      filePath: resolver.resolve('./app/components/WField.vue'),
      global: options.components?.global,
    });

    addComponent({
      name: `${componentPrefix}FieldArray`,
      filePath: resolver.resolve('./app/components/WFieldArray.vue'),
      global: options.components?.global,
    });

    addComponent({
      name: `${componentPrefix}ErrorMessage`,
      filePath: resolver.resolve('./app/components/WErrorMessage.vue'),
      global: options.components?.global,
    });

    addComponent({
      name: `${componentPrefix}FormWizard`,
      filePath: resolver.resolve('./app/components/WFormWizard.vue'),
      global: options.components?.global,
    });

...
export default defineNuxtModule<PaletteConfig>({
	...
	setup(options, nuxt) {
		...
		// Add runtime plugin
		addPlugin({
			src: resolver.resolve('./app/plugins/palette-client.ts'),
			mode: 'client'
		})
		...
		addImports([
			{
				name: 'usePalette',
				from: resolver.resolve('./app/composables/usePalette')
			},
			{
				name: 'usePaletteProvider',
				from: resolver.resolve('./app/composables/usePaletteProvider')
			},
			{

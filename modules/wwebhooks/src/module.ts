...
export default defineNuxtModule<WebhooksModuleOptions>({
  ...
  setup(options, nuxt) {
    ...
    addServerHandler({
      ...
      handler: resolver.resolve('./runtime/server/api/stripe.post'),
    });

    addServerHandler({
      route: '/api/webhooks/github',
      handler: resolver.resolve('./runtime/server/api/github.post'),
    });

    addServerHandler({
      route: '/api/webhooks/slack',
      handler: resolver.resolve('./runtime/server/api/slack.post'),
    });

    addServerHandler({
      route: '/api/webhooks/custom',
      handler: resolver.resolve('./runtime/server/api/custom.post'),
    });
